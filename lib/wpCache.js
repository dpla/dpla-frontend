...
export async function cachedSafeFetch(url, options) {
  ...
  const isDraftMode = options?.headers?.Authorization != null;
  const isCacheable =
  ...
  // Network failure — serve stale data if available, rather than propagating
  // the error and showing users a 500 page during WP Engine maintenance.
  if (cached) {
