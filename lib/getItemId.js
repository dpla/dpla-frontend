import { DPLA_ITEM_ID_REGEX } from "../constants/items.js";
...
const getItemId = (source) => {
  ...
  if (Array.isArray(references)) {
    ...
    // extractItemId returns the whole URL when it finds no item path,
    // so check the shape before trusting it over the citation
    if (id && DPLA_ITEM_ID_REGEX.test(id)) return id;
  }
