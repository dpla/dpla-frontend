import { retryAfterSeconds } from "./retryPolicy.js";
...
/** Appends one query param with `?` or `&`. */
function withQueryParam(url, param) {
  return url.includes("?") ? `${url}&${param}` : `${url}?${param}`;
}

/** Appends `context=edit` for WP draft-mode fetches. */
export function wpDraftUrl(url) {
  return withQueryParam(url, "context=edit");
}

/**
 * Appends `acf_format=standard`.
 * Without it ACF returns image fields as attachment IDs.
 * Use on any request whose `acf` key we read.
 */
export function wpAcfUrl(url) {
  return withQueryParam(url, "acf_format=standard");
}
...
/** Cancels open fetch bodies */
export async function cancelBodies(...fetchResponses) {
  await Promise.allSettled(fetchResponses.map((r) => r?.body?.cancel?.()));
}

/** True for a fetch timeout or connection error */
...
/**
 * Sets 503 + Retry-After
 * Cancels fetch bodies
 * Logs a "[load-shed]" line
 * Returns the Retry-After seconds
 *
 * @param {import('http').ServerResponse} res
 * @param {...(Response|null|undefined)} fetchResponses - the first is logged; pass null for a network failure
 * @returns {Promise<number>}
 */
export async function markUpstreamUnavailable(res, ...fetchResponses) {
  await cancelBodies(...fetchResponses);
  const retryAfter = retryAfterSeconds();
  res.statusCode = 503;
  res.setHeader("Retry-After", String(retryAfter));
  const [first] = fetchResponses;
  let upstream = "-";
  if (first === null) upstream = "network";
  else if (first) upstream = first.status;
  const path = res.req.url.split("?")[0];
  console.warn(
    `[load-shed] 503 ${res.req.method} ${path} upstream=${upstream} retry-after=${retryAfter}`,
  );
  return retryAfter;
}

/**
 * markUpstreamUnavailable() plus the props ServiceUnavailable expects.
 *
 * @param {import('http').ServerResponse} res
 * @param {...(Response|null|undefined)} fetchResponses
 * @returns {Promise<{ props: { temporarilyUnavailable: true, retryAfter: number } }>}
 */
export async function upstreamUnavailable(res, ...fetchResponses) {
  const retryAfter = await markUpstreamUnavailable(res, ...fetchResponses);
  return { props: { temporarilyUnavailable: true, retryAfter } };
}
