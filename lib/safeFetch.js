...
export async function safeFetch(url, options) {
  try {
    ...
    // Log before returning null so the original cause (e.g. ConnectTimeoutError)
    // is visible in Sentry/CloudWatch rather than silently swallowed.
    console.error(`[safeFetch] Network error fetching ${url}:`, err);
    return null;
  }
}
