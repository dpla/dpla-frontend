...
import { safeFetch, safeJson, cancelBodies } from "./safeFetch.js";
...
export const loadExhibition = async (slug) => {
  return slug === "exhibitions" ? null : await loadExhibitFile(`${slug}.json`);
};
...
export const loadDplaItem = async (dplaItemId) => {
  ...
  const dplaApiRes = await safeFetch(itemUrl);
  if (!dplaApiRes?.ok) {
    await cancelBodies(dplaApiRes);
    return null;
  }
  return safeJson(dplaApiRes);
};
