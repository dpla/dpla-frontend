...
export async function getServerSideProps(context) {
  ...
  const subsection = exhibitPageSubpages(exhibit, section).find(
    (subsection) => subsection.slug === context.params.subsectionSlug,
  );
  ...
  subsection.page_blocks = await processPageBlocks(
    subsection,
    context.query.item,
  );

  if (!subsection.page_blocks.find((block) => block.isActive))
    return { notFound: true };

  const sections = exhibitParentPages(exhibit);
  const subsections = exhibitPageSubpages(exhibit, section);
  const previousSubpage =
    subsection.order !== subsections[0].order
  ...
  if (!isLastPage) {
    if (subsections[subsections.length - 1].slug === subsection.slug) {
      ...
      const nextSubsection = subsections.find(
        (s) => s.order === subsection.order + 1,
      );
      Object.assign(nextQueryParamsAndTitle, {
        queryParams: {
          exhibitionSlug: exhibit.slug,
