...
6. If the upstream returns 404 → `404`; 400 → `400`; 5xx or unreachable → `503` with a `Retry-After` header. Single-item requests (`?single=1`) also return `503` when the body is not the API's JSON shape; multi-item responses are streamed through unchecked. Any other failure → `502`.
...
| Upstream DPLA API 5xx or unreachable; or, for single-item requests, a body not in the API's JSON shape | 503 + `Retry-After` | `{ "error": "Upstream service unavailable." }` |
| Upstream DPLA API returned 400 (for example, more than 500 ids) | 400 | `{ "error": "Bad request." }` |
| Other upstream error | 502 | `{ "error": "Upstream service error." }` |
