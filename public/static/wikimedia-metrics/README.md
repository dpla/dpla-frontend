...
`metrics.js` depends on `google.visualization`, which is loaded asynchronously by the Google Charts loader. To guarantee the correct order, the Next.js page uses:
...
`metrics.js` then calls `google.charts.load(...)` at parse time (before `initMetrics()` runs), storing the returned promise in `chartsReady`. Panels that auto-open on load (`autoOpen=true`) wait on `chartsReady` before drawing charts; panels opened by user click can call `google.visualization` directly because by then the library is guaranteed to be loaded.
