...
function follower() {
  ...
  app.prepare().then(() => {
    ...
    if (process.env.SITE_ENV === "user" || process.env.SITE_ENV === "pro") {
      server.get("/donate", donate(app));
      server.get("/donate/thank-you", thankYou(app));
      server.get(["/contact", "/contact-us"], contact(app));
      server.get("/wp-content/*", wpContent());
      server.post("/mailchimp", mailchimp());
      server.post("/g/contact", doContact());
      server.post("/g/feedback", feedback());
    }

    server.get("*", catchall(handle));
