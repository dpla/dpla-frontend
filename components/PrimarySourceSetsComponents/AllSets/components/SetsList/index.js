            ...
            {sets.itemListElement.map(set => <li className={`${css.itemWrapper} ${utils.colXs6} ${utils.colMd4}`}
                ...
                <div className={css.item}>
                    <Link
                        ...
                        as={{
                            pathname: `/primary-source-sets/${extractSourceSetSlug(set["@id"])}`,
                    ...
                    <div className={css.itemContent}>
                        <Link
                            href={`/primary-source-sets/set?set=${extractSourceSetSlug(set["@id"])}`}
                        ...
                        <ul className={css.timePeriod}>
                            {set.about
                                .filter(({disambiguatingDescription}) => disambiguatingDescription === "Time Period")
                                .map((tag, i, tags) => <li className={css.tag} key={`${tag}—${i}`}>
                                    <Link
                                        href={{
                                            pathname: "/primary-source-sets", query: Object.assign({}, router.query, {
                        ...
                        <ul className={css.tags}>
                            {set.about
                                .filter(({disambiguatingDescription}) => disambiguatingDescription === "Subject")
                                .map((tag, i, tags) => <li key={tag.name} className={css.tag}>
                                    <Link
                                        href={{
                                            pathname: "/primary-source-sets",
                                            query: {...router.query, subject: mapSubjectNameToSlug(tag.name)}
                                        }}
                                        className="hover-underline" title={tag.name}
                                    >
                                        {tag.name.replace("&amp;", "&")}
