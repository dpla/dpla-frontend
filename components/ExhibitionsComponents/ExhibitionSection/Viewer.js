...
import ChevronThickBlack from "components/svg/chevron-thick-black";
...
class Viewer extends React.Component {
    ...
    render() {
        ...
        const activeBlockIndex = pageBlocks.findIndex(block => block.isActive);
        const activeBlock = pageBlocks[activeBlockIndex];
        const previousBlock = activeBlockIndex - 1 >= 0
            ? pageBlocks[activeBlockIndex - 1]
            : null;
        const nextBlock = activeBlockIndex + 1 < pageBlocks.length
            ? pageBlocks[activeBlockIndex + 1]
            : null;
