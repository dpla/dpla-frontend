...
const dplaLink = query => {
    const subject = query.subject ? [`${query.subject}`, `${decodeURIComponent(LOCALS[LOCAL_ID].subjectFacet)}`].join("|") : `${decodeURIComponent(LOCALS[LOCAL_ID].subjectFacet)}`
    let newQuery = {
        q: query.q, subject
    };
...
function AboutLocal({items, query}) {
    const itemChildren = items.map((item) => {
            ...
            const titleText = item.title ? truncateString(item.title, 150) : UNTITLED_TEXT;
            return (
                <li key={`ab_${item.id}`} className={css.item}>
                    ...
                    <div className={css.itemInfo}>
                        ...
                        {(item.date || item.creator) && <p className={css.itemAuthorAndDate}>
                            {item.date && <span>{item.date.displayDate}</span>}
                            {item.date && item.date.displayDate && item.creator && <span> · </span>}
    ...
    return <section className={`${css.wrapper}`}>
        <div className={utils.container}>
            <h1 className={css.header}>
                <a className="external" href={dplaLink(query)}>Explore related items in DPLA</a>{" "}
                including:
            </h1>
