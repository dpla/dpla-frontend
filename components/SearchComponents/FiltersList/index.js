...
import {
    possibleFacets, mapURLPrettifiedFacetsToUgly, mapFacetsToURLPrettified
...
const clearAllFacets = query => {
    ...
    delete duped["q"];
    delete duped["page"];
    possibleFacets.forEach(facet => delete duped[mapFacetsToURLPrettified[facet]]);
    return duped;
};
...
const Filter = ({name, queryKey, router}) => {
...
class FiltersList extends React.Component {
    render() {
        ...
        const hasFacets = Object.keys(query).some(queryKey => possibleFacets.includes(mapURLPrettifiedFacetsToUgly[queryKey]) || queryKey === "tags");
        if (!hasFacets) {
            return <></>
        }
        const filterChildren = Object.keys(query).map((queryKey) => {
            const value = joinIfArray(query[queryKey], "|");
            if (possibleFacets.includes(mapURLPrettifiedFacetsToUgly[queryKey]) || queryKey === "q" || queryKey === "tags") {
                return (value && value.split("|").map((paramValue) => {
                    const name = queryKey !== "after" && queryKey !== "before" ? paramValue.replace(/"/g, "") : paramValue;
                    return (
                        <Filter
                            queryKey={queryKey}
                            key={queryKey}
        ...
        return (
            <div className={css.filtersListWrapper}>
                <div className={`${showFilters ? css.isOpen : ""} ${css.filtersList} ${utils.container}`}>
