...
export function Lists({lists}) {
    return (<ul className={`${css.lists} ${utils.colXs12}`}>
            {lists.map((l) => <li key={`l_${l.uuid}`}>
                <List
                    uuid={l.uuid}
                    name={l.name}
...
export function ListNote() {
    return (<div className={`${css.note} ${utils.colXs12}`}>
        <p>
...
export function ListsEmpty() {
    return (<div className={`${css.empty} ${utils.colXs12}`}>
        <h2 className={css.contentTitle}>Create your lists</h2>
        <p>
            Create lists from our{" "}
            <Link href="/browse-by-topic">topics</Link> or{" "}
...
export function ListCheckbox({
                                 list, shouldDisable, isChecked, onCheckList
                             }) {
    return (<li>
        <label
