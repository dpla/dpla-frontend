...
| Variable | Description |
|---|---|
| `NEXT_PUBLIC_GA_TRACKING_ID` | The site's own GA4 measurement ID (`G-…`) *(optional)* |
| `NEXT_PUBLIC_GA_NETWORK_ID` | Shared GA4 measurement ID that every site also reports to *(optional)* |
| `NEXT_PUBLIC_GA_LEGACY_EVENTS` | Set to `false` to stop the legacy GA event. On by default. Set through the `GA_LEGACY_EVENTS` build arg *(optional)* |
| `SENTRY_DSN` | Server-side Sentry DSN. Set by the ECS task *(optional)* |
| `SENTRY_ENVIRONMENT` | Server-side environment tag, default `development`. Set by the ECS task *(optional)* |
...
Menus come from the `dpla/v2/menus/<menu-slug>` route in the DPLA mu-plugin on that host, not from a menu plugin. The user site reads the `user-site` menu and the pro site reads `pro-site`.
...
`lib/buildGaEvent.js` builds item events and `lib/gtag.js` sends them. Each action sends two events: a GA4-shaped one and the legacy one existing reports use.

| Event | Fired from |
|---|---|
| `item_view` | Item page load and prev/next navigation |
| `click_through` | Click on an item's link to the partner site (item page, search results, Browse by Topic, lists, primary source pages) |
| `browse_item` | Click on an item in Browse by Topic |
| `exhibition_item_view` | Exhibition viewer and exhibition home page |
| `primary_source_view` | Primary source page |

Every event carries the parameters below, each cut to GA4's 100-character limit. GA4 drops a blank field, and reports show it as `(not set)`.

| Parameter | Value |
|---|---|
| `dpla_id` | DPLA item ID. Named to stay clear of GA4's own `item_id` dimension |
...
| `topic`, `subtopic` | Topic browse slugs. On every event from a Browse by Topic page |
